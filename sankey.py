...
pv = pd.read_csv("Git-T/TPPEE/pv.csv")
...
total_consumption_value = sectors_df.loc[sectors_df['Sector'] == 'TOTAL', 'Consumption [kWh/year]'].iloc[0]
sectors = []
sub_sectors = {}
current_sector = None
...
for index, row in sectors_df.iterrows():
    ...
    if name.isupper():
        ...
        if current_sector:
            sub_sectors[current_sector].append({'name': name, 'consumption': consumption})
            if name not in node_map:
                node_map[name] = len(nodes_labels)
                nodes_labels.append(label)
